    ...
    def run(self):
        ...
        while True:
            ...
            if key == Keyboard.LEFT:
                ...
                self.printHelp()
            elif key == ord('O'):   # Testing the recovery loop
               
                self.printInertialUnit()
                fsv = []  # force sensor values

                fsv.append(self.fsr[0].getValues())
                print(f"Left foot sensor values: {fsv[0]}")
                fsv.append(self.fsr[1].getValues())
                print(f"Right foot sensor values: {fsv[1]}")
                left = []
                right = []
                ...
                print(f"Left foot forces: {left}")
                print(f"Right foot forces: {right}")
                self.printInertialUnit()
                rpy = self.inertialUnit.getRollPitchYaw()
                if rpy[1]<0:
                    print("front fall detectd")
                
                print(f"inertial sensor = {rpy[1]}")
                ...
                for i in range(len(left)):
                    ...
                    if num1 > 0:
                        print(f"Array 1: {num1} is greater than zero.")
                    elif num1 < 0:
                        print(f"Array 1: {num1} is less than zero.")
                    else:
                        print(f"Array 1: {num1} is equal to zero.")

                    if num2 > 0:
                        print(f"Array 2: {num2} is greater than zero.")
                    elif num2 < 0:
                        print(f"Array 2: {num2} is less than zero.")
                    else:
                        print(f"Array 2: {num2} is equal to zero.")

                        if rpy[1] >= 0 :  #back sensors of both feet
                            self.printInertialUnit()
                            rpy = self.inertialUnit.getRollPitchYaw()
                            print(f"inertial sensor = {rpy[1]}")
                            if rpy[1]>=0.03:
                                print("Front Fall detected, front recovery initiated")
                                self.startMotion(StandUpFromFront)
                            elif rpy[1]<=-0.08:
                                print("Back Fall detected, back recovery initiated")
                                self.startMotion(StandUpFromback)    # This knimatic should be added
