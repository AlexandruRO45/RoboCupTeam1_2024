    ...
    def run(self):
        ...
        while True:
            ...
            if robot.step(self.timeStep) == -1:
                ...
                def get_sensor_data():
                    return {
                        "roll": 0.5,  # Example value
                        "yaw": 0.8,   # Example value
                        "pitch": 0.3, # Example value
                        "gyroscope": 0.6,  # Example value
                        "pressure": 0.9   # Example value
                         }
                ...
                while robot.step(timestep) != -1:
                    ...
                    with open(csv_filename, "w", newline="") as csvfile:
                        fieldnames = ["Timestamp", "Roll", "Yaw", "Pitch", "Gyroscope", "Pressure"]
                        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                        writer.writeheader()
                    
                        while True:
                            # Get current timestamp (minutes and seconds only)
                            timestamp = datetime.datetime.now().strftime("%M:%S")
                    
                            # Simulate getting sensor data (replace with actual data)
