    ...
    def run(self):
        ...
        while True:
            ...
            if robot.step(self.timeStep) == -1:
                ...
                # Initialize PID gains (tune these values experimentally)
                Kp_orientation = 5
                Ki_orientation = 0.018
                Kd_orientation = 0.0045

                Kp_distance = 0.2
                Ki_distance = 0.01
                Kd_distance = 0.05

                prev_orientation_difference = 0
                prev_distance_error = 0
                I_term_orientation = 0
                I_term_distance = 0
                ...
                csv_filename = "TunnedPID_New_2.csv"
                with open(csv_filename, "w", newline='') as csvfile:
                    writer = csv.writer(csvfile)
                    writer.writerow(['Timestamp', 'Yaw', 'Distance'])
            ...
            # Calculate orientation difference (normalized to [-pi, pi])
                        ...
                        def main():
                            ...
                            acce= supervisor.getDevice("accelerometer")
                        # Obtain position and orientation data
                        position = gps.getValues()
                        orientation = imu.getRollPitchYaw()
                        accel= acce.getValues()
                        
                        ori_m=orientation
                        acce1_m=accel
    
                        # Calculate direction vector to waypoint
                        direction = [waypoint[0] - position[0], waypoint[1] - position[1]]
                        desired_orientation = math.atan2(direction[1], direction[0])
    
                        # Compute orientation difference (normalized to [-pi, pi])
                        ...
                        writer.writerow([robot.getTime(), orientation_difference, distance])
                        print(f"Sensor readings saved to {csv_filename}")
                        ...
                        P_term_orientation = Kp_orientation * orientation_difference
                        I_term_orientation += Ki_orientation * orientation_difference * dt
                        D_term_orientation = Kd_orientation * (orientation_difference - prev_orientation_difference) / dt
                        turn_speed = P_term_orientation + I_term_orientation + D_term_orientation #PAY ATTENTION TO THIS PART (ADD DIFFERENT SPEEDS)
    
                        # Distance control
                        distance_error = distance - 0.5  # Set a desired distance (e.g., 0.5 units)
                        P_term_distance = Kp_distance * distance_error
                        I_term_distance += Ki_distance * distance_error * dt
                        D_term_distance = Kd_distance * (distance_error - prev_distance_error) / dt
                        forward_speed = P_term_distance + I_term_distance + D_term_distance   #PAY ATTENTION TO THIS PART (ADD DIFFERENT SPEEDS)
                        ...
                        if abs(orientation_difference) > 0.05:
                            ...
                            if orientation_difference > 0:
                                if turn_speed >=0.6:
                                    ...
                                    print("Robot turning left 40")
                                elif turn_speed <0.6:
                                    self.startMotion(self.turnLeft20)
                                    print("Robot turning left 20")
                                    print(f"P_term_orientation = {P_term_orientation} ")
                            else:
                                if turn_speed <=- 0.6:
                                    self.startMotion(self.turnRight40)
                                    print("Robot turning right 40")
                                elif turn_speed >- 0.6:
                                    self.startMotion(self.turnRight20)
                                    print("Robot turning right 20")
                                    print(f"P_term_orientation = {P_term_orientation} ")
                        ...
                        prev_orientation_difference = orientation_difference
                        prev_distance_error = distance_error
    
                    # Get distance sensors (assuming you have two named "my_distance_sensor" and "my_other_distance_sensor")
                    #distance_sensor = robot.getDevice("ps0")
                    #other_distance_sensor = robot.getDevice("ps1")
                    #distance_sensor.enable(64)  # Set the time step (64 ms)
                    #other_distance_sensor.enable(64)
