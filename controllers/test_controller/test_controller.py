...
class Nao (Robot):
    ...
    def run(self):
        ...
        # while robot.step(self.timeStep) != -1 and not self.forwards.isOver():
        #     pass  # Wait for the first forward motion to finish
        #
        # self.startMotion(self.forwards)
        # while robot.step(self.timeStep) != -1 and not self.forwards.isOver():
        #     pass  # Wait for the second forward motion to finish
        #
        # counter = 100
        # self.startMotion(self.forwards)
        # while robot.step(self.timeStep) != -1 and counter > 0:
        #     counter -= 1

        # Kick motion
        self.startMotion(self.shoot)
        while robot.step(self.timeStep) != -1 and not self.shoot.isOver():
            pass  # Wait for the kick motion to finish
